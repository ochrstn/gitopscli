...
class DeletePreviewCommand(Command):
    ...
    def execute(self) -> None:
        ...
        with GitRepo(team_config_git_repo_api) as team_config_git_repo:
            ...
            preview_namespace = gitops_config.preview_config.get_preview_name(preview_id)
            ...
            application_name = gitops_config.preview_config.application_name
            if not preview_folder_exists:
                ...
                logging.info(
                    "No preview environment for '%s' and preview id '%s'. I'm done here.", application_name, preview_id,
                )
                return

            self.__commit_and_push(
                team_config_git_repo,
                f"Delete preview environment for '{application_name}' and preview id '{preview_id}'.",
            )
